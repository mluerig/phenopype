...
def watershed(
    ...
    ## find child-contours
    contours = find_contours(watershed_mask, retrieval="ccomp", min_nodes=0)
   
    ## fill contours
    image_watershed = np.zeros(watershed_mask.shape, np.uint8)  
    if len(contours)==0:
        print("Watershed algorithm returned no contours - change settings")
    elif len(contours[contours["order"] == "child"]) == 0: 
        print("Watershed algorithm returned too small contours - change settings")
    else:
